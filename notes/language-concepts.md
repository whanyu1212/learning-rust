...
- [Variables & Mutability](./rust-variables.md) — `let`, immutable-by-default, `mut`, shadowing, `const`/`static`.
- [Data Types](./data-types.md) — scalars, collections, `Option`/`Result`, and custom types.
- [Ownership](./rust-ownership.md) — own vs borrow, `&` vs `&mut`, and why cycles are hard by default.
